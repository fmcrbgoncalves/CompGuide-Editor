...
public class EditCPGPage3 extends AbstractOWLWizardPanel {

	public static final String ID = "EditCPGPage3";
	...
	public Set<String> intendedusers_set;

	public EditCPGPage3(OWLEditorKit owlEditorKit, String cpg_individual) {
	...
	public void aboutToDisplayPanel() {
		for (String a : this.intendedusers_set) {
			model.addElement(a);
		}
		list.setModel(model);
	...
	public void getCPGData(String cpg_individual_name) {
		this.intendedusers_set = new HashSet<String>();
		...
		try {
			...
			OWLDataProperty intendedusers_dataproperty = getOWLModelManager().getOWLDataFactory().getOWLDataProperty(
					IRI.create(getOWLModelManager().getActiveOntology().getOntologyID().getOntologyIRI()
							+ "#intendedUser"));

			Set<OWLLiteral> intendedusers_literal_set = new HashSet<OWLLiteral>();

			intendedusers_literal_set = scope_individual.getDataPropertyValues(intendedusers_dataproperty,
					getOWLModelManager().getActiveOntology());

			for (OWLLiteral a : intendedusers_literal_set) {
				this.intendedusers_set.add(a.getLiteral());
			}
		} catch (Exception e) {
