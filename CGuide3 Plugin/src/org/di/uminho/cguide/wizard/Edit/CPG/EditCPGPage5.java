...
public class EditCPGPage5 extends AbstractOWLWizardPanel {

	public static final String ID = "EditCPGPage5";

	public static final String title = "Insert Scope Data Required";

	private JLabel speciality_label;
	...
	public Set<OWLNamedIndividual> all_speciality, cpg_speciality;

	public EditCPGPage5(OWLEditorKit owlEditorKit, String cpg_individual) {
		super(ID, title, owlEditorKit);
		fixcpg_speciality(cpg_individual);
	}

	protected void createUI(JComponent parent) {
		setInstructions(
				"Please select the Clinical Specialities for this Clinical Practice Guideline and click in the Add button.\nYou can also select a content from the added list and remove it.");
		...
		speciality_label = new JLabel("Clinical Speciality:");
		...
		parent.add(speciality_label, gbc_lblSpecialities);
	...
	public void aboutToDisplayPanel() {
		for (OWLNamedIndividual a : this.all_speciality) {
			model_total.addElement(a.getIRI().getFragment());
		}
		for (OWLNamedIndividual a : this.cpg_speciality) {
	...
	public Set<OWLNamedIndividual> getClinicalSpeciality() {
		...
		OWLClass cs = getOWLModelManager().getOWLDataFactory().getOWLClass(IRI.create(
				getOWLModelManager().getActiveOntology().getOntologyID().getOntologyIRI() + "#ClinicalSpecialty"));
	...
	public Set<OWLNamedIndividual> getcpg_specialities(String cpg_individual_name) {
		...
		try {
			...
			OWLObjectProperty hasClinicalSpecialty_objectproperty = getOWLModelManager().getOWLDataFactory()
					.getOWLObjectProperty(
							IRI.create(getOWLModelManager().getActiveOntology().getOntologyID().getOntologyIRI()
									+ "#hasClinicalSpecialty"));

			Set<OWLIndividual> clinicalspecialities_set = scope_individual.getObjectPropertyValues(
					hasClinicalSpecialty_objectproperty, getOWLModelManager().getActiveOntology());

			for (OWLIndividual a : clinicalspecialities_set) {
	...
	public void fixcpg_speciality(String cpg_individual) {
		this.cpg_speciality = getcpg_specialities(cpg_individual);
		this.all_speciality = getClinicalSpeciality();
		for (OWLNamedIndividual a : this.cpg_speciality) {
			try {
				this.all_speciality.remove(a);
			} catch (Exception e) {
			}
		}
	}
