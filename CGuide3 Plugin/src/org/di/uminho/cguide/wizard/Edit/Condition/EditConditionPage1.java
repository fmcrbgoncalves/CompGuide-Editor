...
public class EditConditionPage1 extends AbstractOWLWizardPanel {

	public static final String ID = "EditConditionPage1";
	...
	private String numericalValue = new String(), qualitativeValue = new String(), comparisonOperator = new String(),
			parameterIdentifier = new String(), conditionParameter = new String(), unit = new String();
	private int type;

	public EditConditionPage1(OWLEditorKit owlEditorKit, String condition_individual) {
	...
	public void aboutToDisplayPanel() {
		if (this.type == 0) {
			...
			numericalvalue_ratiobutton.setSelected(true);
			qualitativevalue_ratiobutton.setSelected(false);
			condition_type = 0;
			numericalvalue_textfield.setEnabled(true);
			qualitativevalue_textfield.setEnabled(false);
			...
			numericalvalue_ratiobutton.setSelected(false);
			qualitativevalue_ratiobutton.setSelected(true);
			condition_type = 1;
			numericalvalue_textfield.setEnabled(false);
			qualitativevalue_textfield.setEnabled(true);
		}
		if (!(this.comparisonOperator.isEmpty())) {
			comparisonoperator_comboBox.setSelectedItem(this.comparisonOperator);
		}
		parameteridentifier_textfield.setText(this.parameterIdentifier);
		conditionparameter_textfield.setText(this.conditionParameter);
	...
	public void getCondition_data(String condition_individual_name) {
		...
		Map<OWLDataPropertyExpression, Set<OWLLiteral>> data = condition_individual
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : data.entrySet()) {
			...
			try {
				if (entry.getKey().asOWLDataProperty().getIRI().getFragment().equals("numericalValue")) {
					...
					this.type = 0;
				} else if (entry.getKey().asOWLDataProperty().getIRI().getFragment().equals("qualitativeValue")) {
					this.qualitativeValue = entry.getValue().iterator().next().getLiteral();
					this.type = 1;
				} else if (entry.getKey().asOWLDataProperty().getIRI().getFragment().equals("parameterIdentifier")) {
					this.parameterIdentifier = entry.getValue().iterator().next().getLiteral();
				} else if (entry.getKey().asOWLDataProperty().getIRI().getFragment().equals("conditionParameter")) {
					this.conditionParameter = entry.getValue().iterator().next().getLiteral();
				} else if (entry.getKey().asOWLDataProperty().getIRI().getFragment().equals("unit")) {
		...
		OWLObjectPropertyExpression objectexpression = getOWLModelManager().getOWLDataFactory().getOWLObjectProperty(
				IRI.create(getOWLModelManager().getActiveOntology().getOntologyID().getOntologyIRI()
						+ "#hasComparisonOperator"));

		try {
			OWLIndividual comparison_individual = condition_individual
					.getObjectPropertyValues(objectexpression, getOWLModelManager().getActiveOntology()).iterator()
					.next();
			this.comparisonOperator = comparison_individual.asOWLNamedIndividual().getIRI().getFragment();
		} catch (Exception e) {
		}
	}
}
