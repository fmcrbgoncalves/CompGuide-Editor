...
public class SelectClinicalTaskPage extends AbstractOWLWizardPanel {
	...
	protected void createUI(JComponent parent) {
		setInstructions("Please select the Individual Clinical Task Type you wish to delete.");
	...
	public Object getNextPanelDescriptor() {
		if (plan.isSelected()) {
			choice = 1;
			return DeletePlanPage.ID;
		} else if (action.isSelected()) {
			choice = 2;
			return DeleteActionPage.ID;
		} else if (decision.isSelected()) {
			choice = 3;
			return DeleteDecisionPage.ID;
		} else if (question.isSelected()) {
			choice = 4;
			return DeleteQuestionPage.ID;
		} else if (end.isSelected()) {
			choice = 5;
			return DeleteEndPage.ID;
		} else
			choice = -1;
